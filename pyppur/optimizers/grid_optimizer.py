...
class GridOptimizer(BaseOptimizer):
    ...
    def _generate_random_directions(self, n_features: int, n_directions: int) -> np.ndarray:
        """
    ...
    def _optimize_sequential(
        ...
        **kwargs
        ...
        if initial_directions is not None and initial_directions.shape[0] >= self.n_components:
            for i in range(self.n_components):
                best_directions[i] = initial_directions[i]
            
            # Normalize
            best_directions = best_directions / np.linalg.norm(best_directions, axis=1, keepdims=True)
        ...
        for component in range(self.n_components):
            ...
            if initial_directions is not None and component < initial_directions.shape[0]:
                ...
                objective_args = (X, component+1) if len(kwargs) == 0 else (X, component+1, *kwargs.values())
                
                current_loss = self.objective_func(best_directions[:component+1].flatten(), *objective_args)
            ...
            for iteration in range(self.n_iterations):
                ...
                if iteration == 0 and best_direction is None:
                    ...
                    directions = self._generate_random_directions(n_features, self.n_directions)
                else:
                    # Later iterations: perturb the best direction so far
                    if best_direction is None:
                        best_direction = self._generate_random_directions(n_features, 1)[0]
                ...
                for i, direction in enumerate(directions):
                    ...
                    loss = self.objective_func(best_directions[:component+1].flatten(), *objective_args)
        ...
        objective_args = (X, self.n_components) if len(kwargs) == 0 else (X, self.n_components, *kwargs.values())
