...
def compute_trustworthiness(X_original: np.ndarray, X_embedded: np.ndarray, n_neighbors: int = 5) -> float:
    """
...
def compute_silhouette(X_embedded: np.ndarray, labels: np.ndarray) -> float:
    ...
    if any(counts < 2):
        warnings.warn("Some labels have fewer than 2 samples, silhouette may be undefined")
        return np.nan
    
    return sklearn_silhouette_score(X_embedded, labels)


def compute_distance_distortion(X_original: np.ndarray, X_embedded: np.ndarray) -> float:
    """
...
def evaluate_embedding(
    ...
    # Trustworthiness
    metrics['trustworthiness'] = compute_trustworthiness(X_original, X_embedded, n_neighbors)
