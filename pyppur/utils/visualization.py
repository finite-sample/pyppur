...
def plot_embedding(
    ...
    if X_embedded.shape[1] not in (2, 3):
        raise ValueError("Can only plot 2D or 3D embeddings, "
                       f"got {X_embedded.shape[1]}D")
...
def plot_reconstruction(X, X_recon, n_samples=3):
    ...
    from matplotlib.colors import Normalize
    norm = Normalize(vmin=min(X_viz.min(), X_recon_viz.min()), 
...
def plot_comparison(
    ...
    for i, (name, embedding) in enumerate(embeddings.items()):
        if embedding.shape[1] != 2:
            raise ValueError(f"Can only plot 2D embeddings, got {embedding.shape[1]}D for {name}")
